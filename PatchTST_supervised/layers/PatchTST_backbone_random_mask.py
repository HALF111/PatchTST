...
# Cell
class PatchTST_random_mask(nn.Module):
    def __init__(self, 
                 c_in:int, 
                 context_window:int, 
                 target_window:int, 
    ...
    # @profile
    def forward(self, z, random_len):                                        # z: [bs x nvars x seq_len]
        
        # 先计算用random_len做掩码之后剩余的patch数量
        # 并算出对应的原始序列的长度，以方便计算revin
        masked_patch_num = random_len
        unmasked_patch_num = self.patch_num - masked_patch_num
        unmasked_original_len = (unmasked_patch_num - 2) * self.stride + self.patch_len
        masked_original_len = z.shape[-1] - unmasked_original_len
        # print("mask_patch:", masked_patch_num)
        # print("unmask_patch:", unmasked_patch_num)
        # print("unmask_seq_len:", unmasked_original_len)
        
        # norm
        # 1、先做RevIN归一化
        if self.revin: 
            z = z.permute(0,2,1)
            z = self.revin_layer(z, 'norm', norm_len=unmasked_original_len)
            z = z.permute(0,2,1)
        
        # # * 改成在这里就应该把前面的数据置为0了
        # z[:, :, :masked_original_len] = 0
            
        # do patching
        # 2、做patching分割
        if self.padding_patch == 'end':
            # * padding_patch_layer就是用z中的最后一项再往后多复制几个
            z = self.padding_patch_layer(z)                                # z: [bs x nvars x (seq_len+stride)]
        ...
        # ! 注意：当step小于size的时候，不同patch之间对于相同时间步的数据点是共享内存的！！！！！
        # z = z.unfold(dimension=-1, size=self.patch_len, step=self.stride)   # z: [bs x nvars x patch_num x patch_len]
        # z = z.permute(0,1,3,2)                                              # z: [bs x nvars x patch_len x patch_num]
        # ! 所以应该重写一个unfold，使得各个patch间的数据不共享
        z_lst = []
        for idx in range(0, z.shape[-1] - self.patch_len + self.stride, self.stride):
            cur_z = z[:, :, idx: idx+self.patch_len].clone()  # [bs x nvars x patch_len]
            z_lst.append(cur_z)  # append完成之后一共有patch_num个patch张量
            # print(cur_z.shape)
        # print(masked_patch_num, unmasked_patch_num)
        # print(masked_original_len, unmasked_original_len)
        # print(z.shape[-1])
        z = torch.stack(z_lst, dim=-1).to(z.device)  # z: [bs x nvars x patch_len x patch_num]
        
        # 然后需要将前面部分的patch中的值置为0，同时加上一个额外用于标识当前patch是否被mask住的0或1的值
        # ! 注意：由于unfold后不同patch之间对于相同时间步的数据点是共享内存的，所以应该在unfold之前就把数据mask掉了！！
        # ! 除非把unfold手动改写成不共享内存的形式
        # (1) 先将前面patch置为0
        z[:, :, :, :masked_patch_num] = 0
        # print("1:", z.shape)
        # print(z[0][0][0])
        # (2) 然后生成一个标识为mask还是没有mask的0/1向量
        mask_vector = [0]*masked_patch_num + [1]*unmasked_patch_num
        mask_vector = torch.tensor(mask_vector).to(z.device)
        mask_vector = mask_vector.unsqueeze(0).unsqueeze(0).unsqueeze(0)
        mask_vector = mask_vector.repeat(z.shape[0], z.shape[1], 1, 1)
        # (3) 将mask向量作为额外的一维附加到z上
        z = torch.cat([z, mask_vector], dim=-2)    # z: [bs x nvars x (patch_len+1) x patch_num]
        # print("2", z.shape)
        # print(z[0][0][0])
...
class TSTiEncoder(nn.Module):  # "i" means channel-independent
    def __init__(self, c_in, patch_num, patch_len, max_seq_len=1024,
        ...
        # * 但是如果要加上额外的mask向量的信息，需要把patch_len改成patch_len+1
        self.W_P = nn.Linear(patch_len+1, d_model)        # Eq 1: projection of feature vectors onto a d-dim vector space
        self.seq_len = q_len
