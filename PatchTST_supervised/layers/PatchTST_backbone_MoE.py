...
# Cell
class PatchTST_backbone_MoE(nn.Module):
    def __init__(self, c_in:int, context_window:int, target_window:int, patch_len:int, stride:int, max_seq_len:Optional[int]=1024, 
        ...
        self.all_seq_lens = [48, 96, 192, 336, 512, 720, 1024]
        ...
        if self.pretrain_head:
            ...
            self.head_lst = [self.create_pretrain_head(head_nf, c_in, fc_dropout) for head_nf in self.head_nf_lst]  # custom head passed as a partial func with all its kwargs
        elif head_type == 'flatten':
            # 如果不预训练线性层的话，那么调用Flatten_Head
            # 它会完成：展平 + 线性映射 + dropout （不过这里的dropout默认设置为0）
            # self.head = Flatten_Head(self.individual, self.n_vars, self.head_nf, target_window, head_dropout=head_dropout)
            self.head_lst = [Flatten_Head(self.individual, self.n_vars, head_nf, target_window, head_dropout=head_dropout) for head_nf in self.head_nf_lst]
        
        # 这里MoE的输入和输出都是target_window
        # 也即MoE加在预测头后面
        # flatten + MoE
        # self.flatten = nn.Flatten(start_dim=-2)
        self.moe = MoE(
            # input_size=self.head_nf,
            input_size=target_window,
    ...
    def forward(self, z):                                                   # z: [bs x nvars x seq_len]
        ...
        # 2、将输入数据按照seq_len进行切割
        z_lst = [z_lst[length] for length in self.seq_len_lst]
        ...
        z_lst = [self.backbone_lst[idx](z) for idx, z in enumerate(z_lst)]                                                 # z: [bs x nvars x d_model x patch_num]
        # 4、展平 + 线性映射（+dropout）
        z_lst = [self.head_lst[idx](z) for idx, z in enumerate(z_lst)]                                                    # z: [bs x nvars x target_window] 
