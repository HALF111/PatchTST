...
# Cell
class PatchTST_backbone_attn_weight_corr_dmodel_indiv(nn.Module):
    def __init__(self, 
                 c_in:int, 
                 context_window:int, 
                 target_window:int, 
        ...
        if self.acf_lst is not None:
            new_acf_lst = self.calc_acf(self.acf_lst)
            # 并且将其变成tensor形式
            new_acf_lst = torch.tensor(new_acf_lst).float()  # [channel, patch_num]
        self.new_acf_lst = new_acf_lst
        
        self.n_vars = self.new_acf_lst.shape[0]
        ...
        self.backbone = TSTiEncoder(c_in, patch_num=patch_num, patch_len=patch_len, max_seq_len=max_seq_len,
                                ...
                                data_len=patch_num, n_vars=self.n_vars, acf_lst=new_acf_lst, **kwargs)
    ...
    def forward(self, z, return_mid_embedding=False):                                                                   # z: [bs x nvars x seq_len]
        # norm
        if self.revin: 
            z = z.permute(0,2,1)
        ...
        w_s_lst = self.backbone.encoder.w_s_lst
        L_n = self.backbone.encoder.L_n
        self.new_acf_lst = self.new_acf_lst.float().to(z.device)
        # (2)然后对权重矩阵乘上forecastability先验
        w_s_weights = [w_s.weight.clone() for w_s in w_s_lst]
        w_s_weights = [torch.mul(w_s, self.new_acf_lst[idx, :]) for idx, w_s in enumerate(w_s_weights)]
        ...
        # print("mask_global[:, 0:1, :]:", mask_global[:, 0:1, :])  # [nvars x 1 x patch_num]
...
class TSTiEncoder(nn.Module):  # i means channel-independent
    def __init__(self, c_in, patch_num, patch_len, max_seq_len=1024,
                 ...
                 data_len=None, n_vars=None, acf_lst=None, **kwargs):
        ...
        self.encoder = TSTEncoder(q_len, d_model, n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, norm=norm, attn_dropout=attn_dropout, dropout=dropout,
                                   ...
                                   data_len=data_len, acf_lst=acf_lst, n_vars=n_vars)

        
    def forward(self, x, return_mid_embedding=False) -> Tensor:                                              # x: [bs x nvars x patch_len x patch_num]
        ...
        if return_mid_embedding:
            mid_embedding = torch.reshape(u, (-1, n_vars, u.shape[-2], u.shape[-1]))  # [bs x nvars x patch_num x d_model]
...
class TSTEncoder(nn.Module):
    def __init__(self, q_len, d_model, n_heads, d_k=None, d_v=None, d_ff=None, 
                        ...
                        data_len=None, n_vars=None, acf_lst=None):
        super().__init__()
        ...
        self.w_s_lst = nn.ModuleList()
        for i in range(self.n_vars):
            self.w_s_lst.append(nn.Linear(data_len, 1, bias=False))
        ...
        # ! 发现问题：这里如果用1/N初始化的话，softmax之后的权重可能会非常平均？！！
        # ! 那么不如直接用1来初始化？
        # ! 发现即使用1甚至10来初始化，最终结果还是会把向量中的每个值训练得很平均？
        # ! 但是如果一开始很不平均的话，那么最后也会很不平均？
        for w_s in self.w_s_lst:
            nn.init.constant_(w_s.weight, 1 / data_len)
            # nn.init.constant_(w_s.weight, 10.)
            
            # nn.init.normal_(w_s.weight, mean=0, std=5)
            
            # cur_weight = w_s.weight
            # # 递增式的初始化？？？
            # denum = 2. / (data_len*(data_len+1))
            # # print(cur_weight.shape[1])
            # for i in range(cur_weight.shape[1]):
            #     # cur_weight.data[:, i] = (i+1) * denum
            #     if i > data_len * 3 / 4:
            #         cur_weight.data[:, i] = 30
            #     else:
            #         cur_weight.data[:, i] = 0
        
        # print([w_s.weight for w_s in self.w_s_lst])
        # a = 1/0
        
        # 存储acf_lst
        # 其shape为[nvars, patch_num]
        self.acf_lst = acf_lst
    ...
    def forward(self, src:Tensor, key_padding_mask:Optional[Tensor]=None, attn_mask:Optional[Tensor]=None):
        ...
        self.acf_lst = self.acf_lst.float().to(src.device)
        
        # 然后对权重矩阵乘上forecastability先验
        # ? 这里是否需要做clone？？
        w_s_weights = [w_s.weight.clone() for w_s in self.w_s_lst]
        w_s_weights = [torch.mul(w_s, self.acf_lst[idx, :]) for idx, w_s in enumerate(w_s_weights)]
        
        # 然后将乘上forecastability先验后的中间向量乘上来
        # ! 别忘记这里需要对self.w_s做softmax！！！
        mask_global = [torch.mm(self.v_l, F.softmax(w_s)) for w_s in w_s_weights]  # mask_global: nvars个[patch_num, patch_num]
        mask_global = [torch.mm(tmp_mask, self.L_n) for tmp_mask in mask_global]  # mask_global: nvars个[patch_num, patch_num] or nvars个[max_q_len, q_len]
        mask_global = torch.stack(mask_global)  # 默认为float, shape为[nvars, patch_num, patch_num]
...
class _ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, q:Tensor, k:Tensor, v:Tensor, prev:Optional[Tensor]=None, key_padding_mask:Optional[Tensor]=None, attn_mask:Optional[Tensor]=None, mask_global=None):
        ...
        bs, n_heads, *_ = q.shape
        n_vars, *_ = mask_global.shape

        # Scaled MatMul (q, k) - similarity scores for all pairs of positions in an input sequence
        # * 事实上，我们应该在计算q和k之间的矩阵乘法时额外引入attention的global的mask：
        # * 目前由于我们希望各个attention层共享相同的mask_global，所以是从上面传下来的。
        # * 注意，此时由于我们对每个channel单独施加mask_global了？！！
        # * 所以此时其shape为[nvars x max_q_len x q_len]
        attn_scores = torch.matmul(q, k)  # attn_scores : [bs x n_heads x max_q_len x q_len]
        # print("mask_global[:, 0, :]:", mask_global[:, 0, :])  # [nvars x max_q_len x q_len]
        mask_global = mask_global.unsqueeze(1).repeat(bs // n_vars, n_heads, 1, 1)  # 此时变成了[bs x n_heads x max_q_len x q_len]
