...
# Cell
class PatchTST_backbone_weighted_concat_no_constrain(nn.Module):
    def __init__(self, 
                 c_in:int, 
                 context_window:int, 
                 target_window:int, 
    ...
    def forward(self, z):                                                   # z: [bs x nvars x seq_len]
        ...
        # 添加权重数据
        z = self.weight_mask(z)                                             # z: [bs x nvars x d_model x patch_num]
...
# 计算权重掩码矩阵
# 由于这里我们希望改成并不保证权重是递增的？？
# 所谓no constrain，就是指这里我们并不保证权重是递增的，可能是在会在中间某个部分权重比较高，其他部分比较低之类的。
class WeightMask(nn.Module):
    def __init__(self, data_len, segment_len=None, acf_lst=None) -> None:
        ...
        if segment_len is None:
            self.diag = torch.eye(data_len)
        else:
            self.diag = torch.zeros(data_len, data_len)
            for i in range(data_len):
                for j in range(data_len):
                    import math
                    # 在判断条件时应该按照[1, data_len]的范围来判断，而非[0, data_len-1]来
                    if (i+1) <= segment_len * math.ceil((j+1) / segment_len) and \
                       (i+1) > segment_len * math.ceil((j) / segment_len):
                        self.diag[i][j] = 1
    ...
    def forward(self, x):
        ...
        # 将上三角函数移到device上
        self.diag = self.diag.to(x.device)  # diag: [bs x nvars x (patch_num*d_model)]
        ...
        # print(self.diag.dtype)
        
        # 3.和上三角矩阵L_n做矩阵乘法
        mask = torch.matmul(gate, self.diag)  # mask: [bs x nvars x (patch_num*d_model)]
        
        # # 打印gate和mask等的相关的信息
        # torch.set_printoptions(profile="full")
        # # print(f"self.diag: {self.diag}")
